...
use crate::{
    AnalysisError, GitChange, GitChangeKind, ImpactResult, RepositoryAnalysis, RepositoryConfig,
...
impl GitChangeSet {
    ...
    pub fn discover(root: &Path, base: &str) -> Result<Self, GitError> {
        ...
        let mut changes = parse_name_status(&diff.stdout)?;
...
impl GitDiffAnalysis {
    ...
    pub fn build(root: &Path, base: &str) -> Result<Self, AnalysisError> {
        ...
        let changes = GitChangeSet::discover(root, base)?.excluding(&excluder);
        let virtual_paths = changes.virtual_paths();
        let repository = RepositoryAnalysis::build_with_virtual_files(
            root,
    ...
    while let Some((field, status)) = fields.next() {
        ...
        match status.as_bytes().first().copied() {
            Some(b'R') => {
                ...
                push_rename(&mut changes, first, second);
            }
            Some(b'C') => {
                let (_, second) = fields.next().ok_or(GitError::MalformedStatus { field })?;
                let second = repository_path(second, "reading changed files")?;
            ...
            Some(b'A') => push_if_python(&mut changes, GitChangeKind::Added, first),
            Some(b'D') => push_if_python(&mut changes, GitChangeKind::Deleted, first),
            Some(b'M' | b'T' | b'U' | b'X' | b'B') => {
                push_if_python(&mut changes, GitChangeKind::Modified, first);
            }
...
fn parse_untracked(output: &[u8]) -> Result<Vec<GitChange>, GitError> {
    output
        ...
        .filter_map(|result| match result {
            Ok(path) if is_python(&path) => Some(Ok(GitChange {
                kind: GitChangeKind::Added,
                path,
                previous_path: None,
            })),
...
fn push_rename(changes: &mut Vec<GitChange>, previous: PathBuf, current: PathBuf) {
    match (is_python(&previous), is_python(&current)) {
        (true, true) => changes.push(GitChange {
            kind: GitChangeKind::Renamed,
            path: current,
            previous_path: Some(previous),
        }),
        (true, false) => changes.push(GitChange {
            kind: GitChangeKind::Deleted,
            path: previous,
            previous_path: None,
        }),
        (false, true) => changes.push(GitChange {
            kind: GitChangeKind::Added,
            path: current,
            previous_path: None,
        }),
...
fn push_if_python(changes: &mut Vec<GitChange>, kind: GitChangeKind, path: PathBuf) {
    if is_python(&path) {
        changes.push(GitChange {
            kind,
            path,
